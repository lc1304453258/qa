<template>
	<div class="container-fluid">
	  	...
	  	<div>
  		  	<fieldset class="form-group">
		  		<div class="input-group">
		  			...
		  			<input id="barcode-input" type="text" class="form-control" placeholder="条码" v-model.trim="barCode" @keydown.enter="keydown">
  		  	...
  		  	<fieldset class="form-group">
		  		<div>
		  			<button type="button" class="btn btn-primary btn-sm" @click="keydown">确定</button>
  			...
  			<div class="col-12 col-sm-6 col-md-6 col-lg-4 col-xl-4 m-card-item" v-for="(data, index) in currentStocks">
  				<div class="card bg-light">
				  	...
				  	<div class="card-block" @click="showModal(data, index)">
				    	...
				    	<p class="card-text">退货数量：<span class="text-primary"><b>{{data.quantity}}</b></span></p>
				  	</div>
				  	<div class="card-footer">
		...
		<Modal ref="modal1" title="退货信息" large="true">
			<div slot="body">
				<div class="form-group">
	  				<label>退货数量</label>
	  				<input type="number" class="form-control" v-model="stockOutQuantity">
	  			</div>
	  			<div class="form-group">
  					<label for="positionSelect">货位</label>
  					<div class="v-select-container">
  						<v-select id="positionSelect" label="message" :options="currentStock.inventoryPositions" v-model="currentStock.inventoryPosition"></v-select>
  					</div>
  				</div>
			</div>
			<div slot="footer">
		...
		<Modal ref="modal2" title="选择要退货的库存" large="true">
			<div slot="body">
				<div class="col-12 col-sm-6 col-md-6 col-lg-4 col-xl-4 m-card-item" v-for="(data, index) in datasForSelect">
  					<div class="card bg-light">
				  		...
				  		<div class="card-footer text-center">
				  			<button class="btn btn-primary btn-sm" type="button" @click="select(data)">选定</button>
				  		</div>
					</div>
  				</div>
			</div>
		...
		<!-- <Modal ref="modal3" title="退货信息" large="true">
			<div slot="body">
				<div clas="form-group">
					<label>领用部门</label>
					<div class="v-select-container">
						<v-select label="name" :options="orgnizations" v-model="orgnization"></v-select>
					</div>
				</div>
				<div class="form-group">
	  				<label>领用人</label>
	  				<input type="text" class="form-control" v-model.trim="pickUser">
	  			</div>
	  			<div class="form-group">
	  				<label>领用单号</label>
	  				<input type="text" class="form-control" v-model.trim="pickNo">
	  			</div>
			</div>
			<div slot="footer">
				<div>
		...
		methods: {
			...
			save: function () {
				...
				$.ajax({
					type : "post",
					dataType : "json",
					url : Common.getApiURL('StockOutResource/saveRedByStockOutScan/') + _self.documentTypeId,
		...
		watch: {
			"stockOutQuantity" : {
				handler: function (to, from) {
					if (to) {
						if (to <= 0) {
							Notify.error("错误", "退货数量必须大于0", false);
