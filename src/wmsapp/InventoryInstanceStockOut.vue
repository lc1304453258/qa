    ...
    <div class="container-fluid">
        <Navbar title="扫描出库">
            <button type="button"
                    ...
                    @click="componentStockOut()">强制出库</button>
            <div class="dropdown"
            ...
            <div v-for="(data, index) in inventoryStockOutDtoList"
                ...
                <div class="card">
                    ...
                    <div class="card-footer"
                         v-if="(data.errorCode == 3 || data.errorCode == 2)">
...
module.exports = {
    ...
    methods: {
        ...
        /**跳转到子件强制出库页面 */
        componentStockOut: function () {
            this.$router.push({
                path: '/desktop/inventoryInstanceStockOutByComponent',
            })
        },
        /**
         * 选择查看其他类型的数据
        ...
        deleteOne: function (index) {
            ...
            for (var i = 0; i < this.inventoryStockOutDtoList.length; i++) {
                if (this.inventoryStockOutDtoList[i].inventoryId == inventoryId) {
                    if (this.inventoryStockOutDtoList[i].scanQuantity == quantity && (errorCode == 2 || errorCode == 3)) {
        ...
        stockOutCheck: function () {
            ...
            if (_self.inventoryStockOutDtoList.length > 0 && updateIventoryInstanceDtos.length > 0 && _self.isRequire ==
                ...
                StockOutGeneratorResource.stockOutCheck(updateIventoryInstanceDtos, _self.stockOutPrepareId).then(
                    successData => {
            ...
            data.forEach(function (newData) {
                ...
                for (var index = 0; index < count; index++) {
                    ...
                    if ((existInventoryInstanceStockInDto.epc != null && existInventoryInstanceStockInDto.epc == newData.epc) || (existInventoryInstanceStockInDto.barCode != null && existInventoryInstanceStockInDto.barCode == newData.barCode)) {
                        if (newData.errorCode == 5) {
                            _self.inventoryInstanceStockOutDtoList.splice(index, 1);
                            index--;
                            break;
                        }
                        if (isCheck == true) {
                            if (newData.errorCode != 2) {
                                ...
                                existInventoryInstanceStockInDto.finished = null;
                                existInventoryInstanceStockInDto.quantity = newData.quantity;
                                existInventoryInstanceStockInDto.actualQuantity = newData.quantity;
                            }
                        }
        ...
        scanRack: function () {
            var _self = this;
            var updateIventoryInstanceDtos = [];
            if (_self.tagNo.length == 0) {
                Notify.error('提示', '请输入手工单号', false);
                return;
            }
            for (var i = 0; i < _self.inventoryInstanceStockOutDtoList.length; i++) {
            ...
            if (updateIventoryInstanceDtos != null && updateIventoryInstanceDtos.length > 0) {
                ...
                StockOutGeneratorResource.stockOut(updateIventoryInstanceDtos, currentDate, _self.stockOutPrepareId, _self.tagNo).then(successData => {
                    Loading.hide(uuid);
