        ...
        <Navbar title="扫描入库">
            <div class="form-inline">
                <button class="btn btn-info btn-sm"
                        @click="componentStockIn()">强制入库</button>
                <div class="dropdown">
                    <button type="button"
                        ...
                        <div class="row">
                            ...
                            <div class="{'col': (showPictureOfApp == true ? '12' : '9')}">
                                <div class="d-flex justify-content-between">
                                    <div class="p-2">总数量：{{data.quantity}}</div>
                                </div>
                            </div>
                        </div>
                    </div>
...
module.exports = {
    ...
    methods: {
        ...
        /**跳转到子件强制出库页面 */
        componentStockIn: function () {
            this.$router.push({
                path: '/desktop/inventoryInstanceScanRackByComponent',
            })
        },
        /**
         * 选择查看其他类型的数据
        ...
        checkBeforeScanRack: function () {
            ...
            if (updateIventoryInstanceDtos.length > 0 && _self.require == false) {
                _self.require = true;
                StockInResource.stockInCheck(updateIventoryInstanceDtos).then(successData => {
                    _self.require = false;
                    if (successData) {
                    ...
                    for (var k = 0; k < count; k++) {
                        ...
                        if ((existInventoryInstanceStockInDto.epc != null && existInventoryInstanceStockInDto.epc == newData.epc) || (existInventoryInstanceStockInDto.barCode != null && existInventoryInstanceStockInDto.barCode == newData.barCode)) {
                            if (newData.errorCode == 5) {
                                _self.inventoryInstanceDtos.splice(k, 1);
                                count--;
                            } else {
                                if (isCheck == true) {
                                    ...
                                    existInventoryInstanceStockInDto.hasBom = newData.hasBom;
        ...
        save: function (updateIventoryInstanceDtos) {
            ...
            if (updateIventoryInstanceDtos.length > 0 && _self.tagNo != undefined) {
                ...
                StockInResource.stockIn(updateIventoryInstanceDtos, currentDate, _self.tagNo).then(successData => {
                    Loading.hide(uuid);
