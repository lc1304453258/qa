<template>
	<div class="container-fluid">
	  	...
	  	<div>
  		  	...
  		  	<fieldset class="form-group">
		  		<div>
    				...
    				<button type="button" class="btn btn-primary btn-sm" @click="beforeSave">保存</button>
    				<button type="button" class="btn btn-success btn-sm" @click="autoSetPosition">复制货位</button>
	  			</div>
			</fieldset>
		</div>
  			...
  			<div class="col-12 col-sm-6 col-md-6 col-lg-4 col-xl-4 m-card-item" v-for="(data, index) in inventoryInstances">
  				<div class="card bg-light">
				  	...
				  	<div class="card-block"  @click="showModal(data,index)">
				  		...
				  		<p class="card-text" v-if="data.inventoryInstance && data.inventoryInstance.batchNo">存货批号：{{(data.inventoryInstance) ? data.inventoryInstance.batchNo : undefined}}</p>
				    	<p class="card-text">存货条码：{{(data.inventoryInstance) ? data.inventoryInstance.barCode : undefined}}</p>
				    	<p class="card-text">退料数量：<span class="text-primary"><b>{{(data.inventoryInstance) ? data.inventoryInstance.quantity : undefined}}</b></span></p>
			...
			<div slot="body">
				<div>
					<div class="form-group">
  						...
  						<input id="stockInQuantityInput" type="number" class="form-control" v-model="stockInQuantity">
  					</div>
  					<div class="form-group">
  						<label for="positionSelect">货位</label>
  						<div class="v-select-container">
  							<v-select id="positionSelect" label="positionBarCode" :options="positions" v-model="position"></v-select>
  						</div>
  					</div>
				</div>
			</div>
		...
		<Modal ref="modal2" title="提示" large="true">
			<div slot="body">
				未找到该条码对应的存货，需要新建吗？
			</div>
			<div slot="footer">
				<div>
					<button type="button" class="btn btn-primary" @click="modalSave2">确定</button>
  					<button type="button" class="btn btn-danger" @click="modalCancel2">取消</button>
				</div>
			</div>
		</Modal>

		<Modal ref="modal3" title="存货信息" large="true">
			<div slot="body">
				<div clas="form-group">
					<label>条码</label>
				...
				<div class="form-group">
	  				<label>存货名称</label>
	  				<input type="text" class="form-control" v-model.trim="inventoryName">
	  			</div>
	  			<div class="form-group">
	  				<label>规格型号</label>
	  				<input type="text" class="form-control" v-model.trim="inventoryType">
	  			</div>
	  			<div class="form-group">
	  				<label>存货分类</label>
	  				<div class="v-select-container">
	  					<v-select :options="inventoryClasses" v-model="inventoryClass" label="cInvCName"></v-select>
	  				</div>
	  			</div>
			</div>
			<div slot="footer">
				<div>
					<button type="button" class="btn btn-primary" @click="generateInventory">确定</button>
  					<button type="button" class="btn btn-danger" @click="modalCancel3">取消</button>
				</div>
			</div>
		</Modal>

		<Modal ref="modal3" title="单据类型" large="true">
			<div slot="body">
				<select v-model="documentTypeId" class="form-control">
			...
			<div slot="footer">
				<div>
					<button type="button" class="btn btn-primary" @click="modalSave3">确定</button>
  					<button type="button" class="btn btn-danger" @click="modalCancel3">取消</button>
				</div>
			</div>
		</Modal>
	...
	module.exports = {
		...
		methods: {
			...
			autoSetPosition: function () {
				var indexOfEmpty = -1;
				if(this.inventoryInstances == null || this.inventoryInstances.length <= 0){
					Notify.error("提示", "未查询到数据。", false);
					return;
				}
			...
			save: function (index) {
				...
				var uuid = Loading.show();
				$.ajax({
					url : Common.getApiURL("stockInResource/saveRedByInventoryInstance/")+_self.documentTypeId,
					type : "post",
